...
*)
  ...
  let display_message ~is_backtrace ~line_breaks ~severity ~code (msg : Diagnostic.message) ~additional_messages =
    let explication =
      ...
      E.explicate ~line_breaks ~splitting_threshold:5 (main_span @ additional_spans)
    in
  ...
  let display_diagnostic ~line_breaks ~tab_size show_backtrace Diagnostic.{severity; code; message; backtrace; additional_messages} =
    FileReader.run @@ fun () ->
    let msgs =
      Bwd.snoc
        (if show_backtrace then
           Bwd.map (display_message ~is_backtrace:true ~line_breaks ~tab_size ~severity ~code ~additional_messages:[]) backtrace
         else
           Emp)
        (display_message ~is_backtrace:false ~line_breaks ~tab_size ~severity ~code message ~additional_messages)
    in
    vcat_with_pad ~pad:1 (Bwd.to_list msgs)

  module F = Explicator.Make(FileReader)

  let display ?(line_breaks=`Traditional) ?(tab_size=8) ?(show_backtrace = false) diag =
    Notty_unix.output_image (display_diagnostic ~line_breaks ~tab_size show_backtrace diag)

  let interactive_trace ?(line_breaks=`Traditional) ?(tab_size=8) Diagnostic.{code; severity; message; additional_messages; backtrace} =
    let traces =
      FileReader.run @@ fun () ->
      Bwd.snoc
        (backtrace |> Bwd.map (fun msg -> display_message ~line_breaks ~tab_size ~is_backtrace:true ~severity ~code msg ~additional_messages:[]))
        (display_message ~line_breaks ~tab_size ~is_backtrace:false ~severity ~code message ~additional_messages)
