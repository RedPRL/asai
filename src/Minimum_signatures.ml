(** The minimum interface to use diagnostic handlers.
    ...
    Any module implementing {{!module-type:Asai.Reporter.Message}[Reporter.Message]} or {!module-type:Structured_reporter.Message} will also implement this minimum interface. *)
module type Message =
sig
...
(** The minimum interface for libraries and applications to work together.
    ...
    {b If you are the author of a library that uses asai,} it is recommended to wrap your reporter with this signature in the wrapper. For example, if you are using Dune, and your wrapper file is [Cool_library.ml(i)], use this to wrap your [Reporter]:
    {[
    ...
    {[
      Reporter.adopt (Diagnostic.map message_mapper) Cool_library.Reporter.run @@ fun () -> ...
    ]}
