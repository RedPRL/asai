...
module type S = sig
  ...
  val explicate : ?line_breaking:[`Unicode | `Traditional] -> ?block_splitting_threshold:int -> ?blend:(Tag.t -> Tag.t -> Tag.t) -> (Tag.t * Range.t) list -> Tag.t t
  (** Explicate a list of ranges using content from a data reader.
