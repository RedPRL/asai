...
exception Invalid_range of Range.t * String_utils.invalid_range
(** [Invalid_range (range, reason)] means that [range] is an invalid range because of [reason]. This exception will be raised only when the debug mode is enabled. See the [debug] parameter of {!val:Source_marker.S.mark} for enabling the debug mode. *)
...
module type S = sig
  ...
  (** Mark content from a source reader with a list of ranges. This function must be run within [Source_reader.run].
